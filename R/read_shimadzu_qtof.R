...
#' `low + high * 2^32`. The low half is unsigned, but R has no unsigned 32-bit
#' type and reads it as signed, so it is corrected before recombining. Left
#' uncorrected, it would shift half of all flight times by `2^32` (2 to 9 ppm
#' of flight time in the files examined, so twice that in m/z) and read
#' `0x80000000` as `NA`. Conversion is
#' \deqn{mz = ((t-B)/A)^2}
#'
...
#' ~1.5% between polarities on one of the two instruments examined (0.006% on
#' the other), which is ~3% in m/z, so fixed
...
#' header (2 for most scans, 4 for scans containing large values, and
#' occasionally 1). The stored
...
#' @param format_out Class of output. Either `data.table` or `data.frame`.
#' Spectra are long and mixed-type, so `matrix` resolves to `data.table`.
...
#' the accumulation count is missing from the file or differs between its
#' acquisition events, since there is no safe
...
#' contains, each a `data.table` or `data.frame` in
#' long format with columns `scan`, `rt`, `mz` and
#' `intensity`, and a `precursor_mz` column before `mz` for product-ion
