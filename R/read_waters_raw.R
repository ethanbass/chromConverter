...
read_waters_raw <- function(path, format_out = c("matrix", "data.frame"),
...
read_waters_chro <- function(path, format_out = c("matrix", "data.frame"),
                            data_format = c("wide", "long"),
                            read_metadata = TRUE,
                            metadata_format = c("chromconverter", "raw")){
  ...
  if (data_format == "long"){
    ...
    dat <- data.frame(row.names = times, int = int)
  }
