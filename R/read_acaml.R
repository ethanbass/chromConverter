...
#' @param format_out Either \code{data.frame} or \code{data.table}.
...
#' @return A \code{data.frame} or \code{data.table} (according to the value of
#' \code{format_out}) containing sample metadata derived from the supplied ACAML
#' files.
#' @export
read_acaml <- function(paths, find_files,
                       format_out = c("data.frame","data.table"),
                       progress_bar = TRUE, cl = 1){
  format_out <- match.arg(format_out, c("data.frame", "data.table"))
