# Authors and Citation

## Authors

- **Ethan Bass**. Author, maintainer.
  [](https://orcid.org/0000-0002-6175-6739)

- **James Dillon**. Contributor, copyright holder.  
  Author and copyright holder of source code adapted from the
  'Chromatography Toolbox' for parsing 'Agilent' FID files.

- **Evan Shi**. Contributor, copyright holder.  
  Author and copyright holder of source code adapted from 'rainbow' for
  parsing 'Agilent' UV files.

## Citation

Source:
[`inst/CITATION`](https://github.com/ethanbass/chromConverter/blob/v0.10.0/inst/CITATION)

Bass, E. (2026). chromConverter: Chromatographic File Converter.
http://doi.org/10.5281/zenodo.6792521.

    @Manual{,
      title = {chromConverter: Chromatographic File Converter},
      author = {Ethan Bass},
      year = {2026},
      version = {version 0.10.0},
      doi = {10.5281/zenodo.6792521},
      url = {https://ethanbass.github.io/chromConverter/},
    }
